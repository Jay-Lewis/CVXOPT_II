...
beta_gd, error_gd, _, betas_gd = gd.descent(data_stoch_sg_update, subgrad_fn, None, beta_start)
...
beta_start = Variable(torch.zeros(np.shape(X_train)[1], num_c).type(dtype), requires_grad=True)
subgrad_fn = gd.get_torch_subgrad
beta_sgd, error_sgd, _, betas_sgd = gd.descent(subgradient_update, subgrad_fn, None, beta_start)
...
beta_start = Variable(torch.zeros(np.shape(X_train)[1], num_c).type(dtype), requires_grad=True)
subgrad_fn = gd.get_torch_subgrad
beta_svrgd, error_svrg, _, betas_svrg = gd.descent(svrg_update, subgrad_fn, None, beta_start)
